...
from utils.util import get_roc_ap_score
...
flags.DEFINE_string("feature", "True", 'whether to use feature in link prediction')
...
flags.DEFINE_string("num_inducing", '[500]', 'number of inducing points of each layer')
...
flags.DEFINE_integer("n_neighbors", 20, "number of neighbors to be considered in neighbor sampling")
...
# parameter config
with_feature = eval(FLAGS.feature)
...
def log_parameter_settings():
    ...
    tf.logging.info("feature: {}".format(with_feature))
...
def main():
    ...
    graph = Graph(FLAGS.dataset, FLAGS.data_path, K_hop=FLAGS.K_hop, with_feature=with_feature)
    ...
    if FLAGS.sample == "neighbor":
        if with_feature:
            neighbors = graph.get_K_neighbors_prior(FLAGS.n_neighbors)
        else:
            neighbors = graph.get_K_neighbors(FLAGS.n_neighbors)
    else:
        neighbors = None
    ...
    for i in range(FLAGS.steps):
        ...
        if FLAGS.dataset != 'pubmed':
            ...
            false_edges_value = graph.get_false_edges(int(n_pos_edges * 2))
        ...
        """
        if FLAGS.sample == "neighbor" and (i > 0 and i % 50 == 0):
            neighbors = graph.get_K_neighbors(FLAGS.n_neighbors)
            model.update_neighbor_indices(neighbors.shape[0], neighbors)
        """

        if i % 10 == 0:
            Fs_value, Fmeans_value = sess.run([Fs, Fmeans], feed_dict=feed_dict)
            F, Fmean = Fs_value[-1], Fmeans_value[-1]  # [S, N, D]
            auc_sample, ap_sample, auc_mean, ap_mean = get_roc_ap_score(F, Fmean, graph.val_edges, graph.val_edges_false)
            tf.logging.info("acu_sample:{:.6}, ap_sample:{:.6}, "
                            "auc_mean:{:.6f}, ap_mean:{:.6}".format(auc_sample, ap_sample, auc_mean, ap_mean))
            auc_sample, ap_sample, auc_mean, ap_mean = get_roc_ap_score(F, Fmean, graph.test_edges,
                                                                        graph.test_edges_false)
            tf.logging.info("Test_acu_sample:{:.6}, Test_ap_sample:{:.6}, "
                            "Test_auc_mean:{:.6f}, Test_ap_mean:{:.6}.".format(auc_sample, ap_sample, auc_mean, ap_mean))
    ...
    tf.logging.info("acu_sample:{:.6}, ap_sample:{:.6}, "
                    "auc_mean:{:.6f}, ap_mean:{:.6}".format(auc_sample, ap_sample, auc_mean, ap_mean))
    auc_sample, ap_sample, auc_mean, ap_mean = get_roc_ap_score(F, Fmean, graph.test_edges, graph.test_edges_false)
    tf.logging.info("Test_acu_sample:{:.6}, Test_ap_sample:{:.6}, "
                    "Test_auc_mean:{:.6f}, Test_ap_mean:{:.6}.".format(auc_sample, ap_sample, auc_mean, ap_mean))
